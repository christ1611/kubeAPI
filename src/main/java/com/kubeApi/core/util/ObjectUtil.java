...
//    }
    ...
    public static <T> T copyObject(Object fromObject, Class<T> clazz) {
        try {
            ...
            String fromJson = objectMapper.writeValueAsString(fromObject);
    ...
    public static <T> T copyObject(Object fromObject, Class<T> clazz, ObjectMapper mapper) {
        try {
            String fromJson = mapper.writeValueAsString(fromObject);
            return mapper.readValue(fromJson, clazz);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
                                    ...
                                    if (!(pt.isAssignableFrom(tdata.get(key).getClass()))) {
                                        if (LocalDate.class.isAssignableFrom(pt)) {
                                            if (StringUtils.isBlank(sVal))
                                                ...
                                                val = LocalDate.parse(sVal );
//                                                val = LocalDate.parse(sVal, TifsDef.DATE_FORMATTER);
                    ...
                    else
                    {
                        if (val instanceof String) {
                            if (BigDecimal.class.isAssignableFrom(clazz))
                                ...
                                return (T) LocalDate.parse(map.get(key).toString());
                            else
                                return clazz.cast(val);
                        }
                        else if (val instanceof LocalDate ld) {
                            if(String.class.isAssignableFrom(clazz))
                                return (T) ld.toString();
                        }
                    }
                }
                else
    ...
    public static <T> T getMapValuePath(Map<String,Object> map, String path, Class<T> clazz)
    {
        return getMapValuePath( map, path, clazz, ">");

    }
    public static <T> T getMapValuePath(Map<String,Object> map, String path, Class<T> clazz, String splitChar)
    {
        if(StringUtils.indexOf(path, splitChar) == -1 || map == null )
        {
            return null;
        }

        String tsplitChar = splitChar;

        if(StringUtils.equals(tsplitChar,".")) tsplitChar = "\\.";

        String[] prr = path.split(tsplitChar);

        Map<String,Object> pm =  map;
        for (int i = 0; i < prr.length; i++) {

            if( i == prr.length -1 )
            {
                return getMapValue(pm, prr[i], clazz);
            }
            else
            {
                if ( pm.get(prr[i]) instanceof Map )
                {
                    pm = (Map) pm.get(prr[i]);
                }
            }
    ...
    public static Object getMapValuePath(Map<String,Object> map, String path)
    {
        return getMapValuePath( map, path, ">");
    }

    public static Object getMapValuePath(Map<String,Object> map, String path, String splitChar)
    {
        if( map == null ) return null;
        if(StringUtils.indexOf(path, splitChar) == -1 )
        {
            return map.get(path);
        }
        String[] prr = null;
        if(StringUtils.equals(".", splitChar))
        {
            prr = path.split("\\.");
        }
        else
        {
            prr = path.split(splitChar);
        }
    ...
    public static void setMapValuePath(Map<String,Object> map, String path, String splitChar, Object value)
    {
        if( map == null ) return ;
        ...
        for (int i = 0; i < prr.length; i++) {
            ...
            {
                if ( !pm.containsKey(prr[i]) )
                {
                    pm.put(prr[i], new HashMap<String, Object>() );
                }
